...
#endif
				int type, offset;
				type   = *relInfo >> 12;					// the upper 4 bits define the type of relocation
				offset = *relInfo & 0xfff;				// the lower 12 bits define the offset
				switch (type)
				{
				case IMAGE_REL_BASED_ABSOLUTE:		break;										// skip relocation
				case IMAGE_REL_BASED_HIGHLOW:
					patchAddrHL = (DWORD *) (dest + offset);								// change complete 32 bit address
					*patchAddrHL += delta;
					break;
#ifdef _WIN64
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
	...
	printf("PE header found.\n");

	printf("PE Image Base size = %ld.\n",old_header->OptionalHeader.SizeOfImage);
	...
	if (result->headers->OptionalHeader.AddressOfEntryPoint != 0) {
		...
		if (DllEntry == 0) {
			printf("Library has no entry point.\n");
			goto error;
		} else {
			printf("Found library Entry point %p.\n",DllEntry);
		}
		printf(">>> Executing DLL entry point.\n");
		// notify library about attaching to process
		successfull = (*DllEntry)((HINSTANCE)code, DLL_PROCESS_ATTACH, 0);
		printf(">>> Left DLL entry point, result %d.\n",successfull);
		if (!successfull) {
			printf("Can't attach library.\n");
			goto error;
		}
...
FARPROC MemoryGetProcAddress(HMEMORYMODULE module, const char *name)
{
	...
	if (directory->Size == 0) { printf("DLL has no export table found\n"); return NULL; }

	exports   = (PIMAGE_EXPORT_DIRECTORY) (codeBase + directory->VirtualAddress);
	if (exports->NumberOfNames     == 0) {	printf("DLL doesn't export any names\n");     return NULL; }
	if (exports->NumberOfFunctions == 0) {	printf("DLL doesn't export any functions\n"); return NULL; }
...
void MemoryFreeLibrary(HMEMORYMODULE mod)
{
	...
	if (module != NULL) 
	{
		...
		if (module->codeBase != NULL) {	VirtualFree(module->codeBase, 0, MEM_RELEASE);}			// release memory of library
